...
export default function MainButton({ right, title, id, className, strokeColor }: { right?: boolean, title: string, id?: string, className: string, strokeColor?: string }) {
  return (
    ...
    >
        <span className={`relative inline-flex justify-center items-center rotate-45 p-3 main-button__icon group-hover:before:scale-200 group-hover:after:scale-150 group-focus:before:scale-200 group-focus:after:scale-150 ${strokeColor ? 'white-borders': ''}`}>
            <svg className={right ? 'rotate-135' : '-rotate-45'} width="11" height="12" viewBox="0 0 11 12" fill={strokeColor || "#1A1B1C"} xmlns="http://www.w3.org/2000/svg">
                <path d="M0.714355 5.99998L10.1429 11.4436V0.556396L0.714355 5.99998Z" fill={strokeColor || "#1A1B1C"}/>
            </svg>
        </span>
